...
def evaluate_with_mlflow(df: pd.DataFrame, best_model: str, client: Anthropic, max_samples: Optional[int] = None):
    """
    ...
    with mlflow.start_run(run_name=f"anthropic_vs_{best_model}"):
        # If max_samples is provided and less than the dataset size, sample the data
        ...
        batch_size = 50  # Adjust as needed based on API rate limits
        all_predictions = []
...
def main():
    ...
    results = evaluate_with_mlflow(df, best_model_name, client, args.max_samples)
