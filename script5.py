...
def get_anthropic_sentiment(text: str, client: Anthropic, model_name: str) -> Dict[str, Any]:
    """
    Get sentiment analysis from Anthropic API using a specific model
    ...
    try:
        ...
        logger.error(f"Error calling Anthropic API with model {model_name}: {e}")
        return {"sentiment": "neutral", "confidence": 0.5}

def compare_claude_models(df: pd.DataFrame, best_model: str, api_key: str, 
                        claude_models: List[str], sample_size: int = 100, 
                        batch_size: int = 10):
    """
    Compare multiple Claude models on sentiment analysis task
    
    Args:
        df: DataFrame with data
        best_model: Name of the best model from the dataset (ground truth)
        api_key: Anthropic API key
        claude_models: List of Claude model identifiers to compare
    ...
    # Sample from dataset for testing
    if sample_size < len(df):
        df_sample = df.sample(sample_size, random_state=42)
    else:
        df_sample = df
        
    logger.info(f"Using {len(df_sample)} samples for model comparison")
    
    # Get ground truth values from the best model
    ground_truth = df_sample[f"{best_model}_issue_sentiment"].values
    texts = df_sample["issue"].values
    
    # Prepare results DataFrame
    comparison_results = []
    
    # Process each Claude model
    for model_display_name, model_id in [(name, CLAUDE_MODELS[name]) for name in claude_models]:
        model_start_time = time.time()
        logger.info(f"Testing model: {model_display_name} ({model_id})")
        ...
        for batch_idx in range(num_batches):
            ...
            logger.info(f"Processing batch {batch_idx+1}/{num_batches} for {model_display_name}")
            
            for text in tqdm(batch_texts, desc=f"{model_display_name} - Batch {batch_idx+1}"):
                try:
                    # Add a small delay to avoid rate limiting
                    time.sleep(0.1)
                    
                    result = get_anthropic_sentiment(text, client, model_id)
                    predictions.append(result["sentiment"])
                    confidences.append(result["confidence"])
                except Exception as e:
                    logger.error(f"Error processing text with {model_display_name}: {e}")
        ...
        for label in labels:
            true_positives = sum((ground_truth == label) & (np.array(predictions) == label))
            false_positives = sum((ground_truth != label) & (np.array(predictions) == label))
            false_negatives = sum((ground_truth == label) & (np.array(predictions) != label))
        ...
        model_results_df = pd.DataFrame({
            ...
            f"{model_display_name}_prediction": predictions,
            f"{model_display_name}_confidence": confidences
        })
        
        model_results_path = f"{model_display_name}_results.csv"
        model_results_df.to_csv(model_results_path, index=False, encoding='utf-8')
        logger.info(f"Results for {model_display_name} saved to {model_results_path}")
    ...
    comparison_path = f"claude_models_comparison_{timestamp}.csv"
    comparison_df.to_csv(comparison_path, index=False, encoding='utf-8')
    logger.info(f"Comparison results saved to {comparison_path}")
    
    # Create comparison visualizations
    create_comparison_visualizations(comparison_df, ground_truth_model=best_model)
    
    return comparison_df

def create_comparison_visualizations(comparison_df: pd.DataFrame, ground_truth_model: str):
    """Create visualizations comparing the performance of different models"""
    # 1. Accuracy comparison
    plt.figure(figsize=(12, 6))
    sns.barplot(data=comparison_df, x='model_name', y='accuracy')
    plt.title(f'Accuracy Comparison (Ground Truth: {ground_truth_model})')
    ...
    plt.figure(figsize=(12, 6))
    sns.barplot(data=comparison_df, x='model_name', y='avg_confidence')
    ...
    plt.figure(figsize=(12, 6))
    sns.barplot(data=comparison_df, x='model_name', y='avg_time_per_sample')
    plt.title('Average Processing Time per Sample')
    plt.xticks(rotation=45)
    plt.ylabel('Time (seconds)')
    plt.tight_layout()
    plt.savefig('model_processing_time_comparison.png')
    plt.close()
    
    # 4. F1 Score comparison by sentiment class
    plt.figure(figsize=(15, 8))
    
    # Melt the dataframe to create a format suitable for grouped bar chart
    f1_columns = [col for col in comparison_df.columns if col.endswith('_f1')]
    sentiment_names = [col.replace('_f1', '') for col in f1_columns]
    
    f1_df = comparison_df[['model_name'] + f1_columns].copy()
    f1_df_melted = pd.melt(
        f1_df, 
        id_vars=['model_name'], 
    ...
    f1_df_melted['sentiment_class'] = f1_df_melted['sentiment_class'].str.replace('_f1', '')
    
    # Create the grouped bar chart
    sns.barplot(data=f1_df_melted, x='model_name', y='f1_score', hue='sentiment_class')
    plt.title('F1 Score by Sentiment Class')
    plt.xticks(rotation=45)
    plt.legend(title='Sentiment Class')
    plt.tight_layout()
    plt.savefig('model_f1_scores_by_class.png')
    plt.close()
...
def evaluate_with_mlflow(df: pd.DataFrame, best_model: str, client: Anthropic, 
    ...
    with mlflow.start_run(run_name=f"anthropic_vs_{best_model}"):
        ...
        mlflow.log_param("anthropic_model", model_name)
        ...
        for batch_idx in range(num_batches):
            ...
            for i, text in enumerate(tqdm(batch_texts, desc=f"Batch {batch_idx+1}/{num_batches}")):
                result = get_anthropic_sentiment(text, client, model_name)
...
def main():
    ...
    parser.add_argument("--compare-models", action="store_true", help="Compare multiple Claude models")
    parser.add_argument("--model", choices=list(CLAUDE_MODELS.keys()), 
                       default="claude-3-7-sonnet", help="Claude model to use")
    parser.add_argument("--comparison-sample-size", type=int, default=100,
                       help="Number of samples to use for model comparison")
    ...
    # If model comparison is requested
    if args.compare_models:
        ...
        model_selection = input("\nEnter the model numbers to compare (comma-separated, or 'all'): ").strip()
        
        if model_selection.lower() == 'all':
            selected_models = available_models
        else:
            try:
                model_indices = [int(x.strip()) - 1 for x in model_selection.split(',')]
                selected_models = [available_models[i] for i in model_indices if 0 <= i < len(available_models)]
            except ValueError:
                logger.warning("Invalid selection. Using all models.")
                selected_models = available_models
        
        # Get sample size for comparison
        sample_size = args.comparison_sample_size
        try:
            sample_size_input = input(f"Enter number of samples for comparison (default: {sample_size}): ").strip()
            if sample_size_input:
                sample_size = int(sample_size_input)
        except ValueError:
            logger.warning(f"Invalid input. Using default sample size of {sample_size}.")
        
        # Configure batch size for comparison
        comparison_batch_size = min(10, args.batch_size if args.batch_size else 50)
        
        # Perform model comparison
        logger.info(f"Comparing Claude models: {', '.join(selected_models)}")
        logger.info(f"Using {sample_size} samples with batch size {comparison_batch_size}")
        ...
        logger.info("Claude Models Comparison Results:")
        for _, row in comparison_results.iterrows():
            logger.info(f"Model: {row['model_name']}")
            logger.info(f"  Accuracy: {row['accuracy']:.4f}")
            logger.info(f"  Avg Confidence: {row['avg_confidence']:.4f}")
            logger.info(f"  Avg Time per Sample: {row['avg_time_per_sample']:.4f} seconds")
    ...
    # Get selected model
    selected_model = CLAUDE_MODELS[args.model]
    
    # Evaluate Anthropic API
    logger.info(f"Evaluating Anthropic API (model: {args.model}) against {best_model_name}...")
